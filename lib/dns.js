 ...
 *
 * この関数の実行例(IPv4): getIpAddressViaDnsLookup("localhost", "IPv4").then((data) => { console.log(data); });
 * 返り値の例: {"address": "127.0.0.1", "family": 4}
 * この関数の実行例(IPv4): getIpAddressViaDnsLookup("localhost", "v4").then((data) => { console.log(data); });
 * 返り値の例: {"address": "127.0.0.1", "family": 4}
 * この関数の実行例(IPv4): getIpAddressViaDnsLookup("localhost", 4).then((data) => { console.log(data); });
 * 返り値の例: {"address": "127.0.0.1", "family": 4}
 *
 * この関数の実行例(IPv6): getIpAddressViaDnsLookup("localhost", "IPv6").then((data) => { console.log(data); });
 * 返り値の例: {"address": "::1", "family": 6}
 * この関数の実行例(IPv6): getIpAddressViaDnsLookup("localhost", "v6").then((data) => { console.log(data); });
 * 返り値の例: {"address": "::1", "family": 6}
 * この関数の実行例(IPv6): getIpAddressViaDnsLookup("localhost", 6).then((data) => { console.log(data); });
 * 返り値の例: {"address": "::1", "family": 6}
*/
async function getIpAddressViaDnsLookup(DDNS_HOSTNAME, IP_ADDRESS_PROTOCOL_TYPE) {
    ...
    try {
        //
        if (!((IP_ADDRESS_PROTOCOL_TYPE === "IPv4" || IP_ADDRESS_PROTOCOL_TYPE === "v4" || IP_ADDRESS_PROTOCOL_TYPE === 4) || (IP_ADDRESS_PROTOCOL_TYPE === "IPv6" || IP_ADDRESS_PROTOCOL_TYPE === "v6" || IP_ADDRESS_PROTOCOL_TYPE === 6))) {
            return null;
        }
        if (IP_ADDRESS_PROTOCOL_TYPE === "IPv4" || IP_ADDRESS_PROTOCOL_TYPE === "v4" || IP_ADDRESS_PROTOCOL_TYPE === 4) {
            options.family = 4;
            DDNS_HOST_IP_ADDRESS = await dns.lookup(DDNS_HOSTNAME, options);
        }
        if (IP_ADDRESS_PROTOCOL_TYPE === "IPv6" || IP_ADDRESS_PROTOCOL_TYPE === "v6" || IP_ADDRESS_PROTOCOL_TYPE === 6) {
            options.family = 6;
            DDNS_HOST_IP_ADDRESS = await dns.lookup(DDNS_HOSTNAME, options);
        }
        // JSONに整形する
        ...
        return DDNS_HOST_IP_ADDRESS.replace(/\\/g, "");
    } catch (err) {
